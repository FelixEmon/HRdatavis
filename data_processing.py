...
def preprocess_data(df):
    ...
    processed_df['员工BG'] = processed_df['组织全路径'].astype(str).apply(lambda x: x.split('/')[0] if pd.notnull(x) and '/' in x else x)
    pc_cols = safe_split_付费渠道(processed_df['付费渠道'])
    processed_df = pd.concat([processed_df, pc_cols], axis=1)
    processed_df.rename(columns={'pc_0': '付费渠道_a', 'pc_1': '付费渠道_b', 'pc_2': '付费渠道_c', 'pc_3': '付费渠道_d'}, inplace=True)
...
def get_filter_options(df_processed):
    """Gets unique values for filter dropdowns from the processed DataFrame."""
    if df_processed is None:
        return {'job_types': [], 'job_titles': []}
    job_types = sorted(df_processed['职位类'].dropna().unique().tolist())
    job_titles = sorted(df_processed['专业职位'].dropna().unique().tolist())
    return {'job_types': job_types, 'job_titles': job_titles}


def filter_dataframe(df_processed, selected_bgs, selected_job_types, selected_job_titles, grade_range):
    """Filters the processed DataFrame based on user selections."""
    if df_processed is None:
        return pd.DataFrame()
    filtered_df = df_processed.copy()
    if selected_bgs:
        filtered_df = filtered_df[filtered_df['员工BG'].isin(selected_bgs)]
    if selected_job_types:
        filtered_df = filtered_df[filtered_df['职位类'].isin(selected_job_types)]
    if selected_job_titles:
        filtered_df = filtered_df[filtered_df['专业职位'].isin(selected_job_titles)]
    if grade_range:
        min_grade, max_grade = grade_range
        filtered_df = filtered_df[filtered_df['专业职级-数字'].notna() & (filtered_df['专业职级-数字'] >= min_grade) & (filtered_df['专业职级-数字'] <= max_grade)]
    return filtered_df


def calculate_channel_metrics(df_filtered, mode='relative'):
    """
    Calculates all channel contribution metrics and breakdowns.
    """
    ...
    talentpool_df = df_filtered[(df_filtered['付费渠道_b'] == '内部团队') & ((df_filtered['简历来源'] == '内部人才盘活') | (df_filtered['简历来源'].isnull()) | (df_filtered['简历来源'] == ''))]
    st_df = df_filtered[(df_filtered['付费渠道_b'] == '交付团队') & (df_filtered['付费渠道_c'] == '人才寻访组')]
    liepin_delivery_df = df_filtered[(df_filtered['付费渠道_b'] == '交付团队') & (df_filtered['付费渠道_c'] == '高招猎聘')]

    hires_counts = {
        '媒体': len(media_df), '伯乐': len(bole_df), '猎头': len(lietou_df),
        '人才库盘活': len(talentpool_df), 'ST': len(st_df), '猎聘': len(liepin_delivery_df)
    }

    if mode == 'absolute':
        total_hires_for_percentage = len(df_filtered)
    else:
        total_hires_for_percentage = sum(hires_counts.values())

    if total_hires_for_percentage == 0:
        for channel_name_key in ['媒体', '伯乐', '猎头', '人才库盘活', 'ST', '猎聘']:
            results[channel_name_key] = { "hires": 0, "percentage": 0, "details_text_list": [f"无{channel_name_key}渠道入职"], "pie_data": {"labels": [], "values": []} }
    ...
    if not media_df.empty and media_hires > 0:
        ...
        else: media_breakdown_text.append("无付费渠道D类细分数据")
    else: media_breakdown_text.append("无媒体渠道入职")
    results['媒体'] = {"hires": media_hires, "percentage": media_percentage, "details_text_list": media_breakdown_text, "pie_data": {"labels": media_pie_labels, "values": media_pie_values}}
    ...
    if not bole_df.empty and bole_hires > 0:
        bole_df_for_breakdown = bole_df[~bole_df['伯乐所在BG'].isin(['nan', '#N/A', '子公司组织']) & bole_df['伯乐所在BG'].notna()].copy()
        if not bole_df_for_breakdown.empty:
            bole_df_for_breakdown['is_本BG'] = (bole_df_for_breakdown['伯乐所在BG'] == bole_df_for_breakdown['员工BG'])
            本BG_count, 其他BG_count = bole_df_for_breakdown['is_本BG'].sum(), len(bole_df_for_breakdown) - bole_df_for_breakdown['is_本BG'].sum()
            ...
            bole_breakdown_text.append(f"本BG: {本BG_perc:.0f}% "); bole_breakdown_text.append(f"其他BG: {其他BG_perc:.0f}% ")
        ...
        else: bole_breakdown_text.append("无有效伯乐BG数据进行细分")
    else: bole_breakdown_text.append("无伯乐渠道入职")
    results['伯乐'] = {"hires": bole_hires, "percentage": bole_percentage, "details_text_list": bole_breakdown_text, "pie_data": {"labels": bole_pie_labels, "values": bole_pie_values}}
    ...
    if not lietou_df.empty and lietou_hires > 0:
        ...
        if not source_counts.empty:
            ...
            for i, (source, count) in enumerate(source_counts.head(top_n).items()):
                ...
                lietou_pie_labels.append(f"{display_source} (TOP{i + 1})"); lietou_pie_values.append(count)
                current_sum += count
            ...
            if other_count > 0:
                ...
                lietou_pie_labels.append("其他猎头"); lietou_pie_values.append(other_count)
            if not lietou_pie_labels and not lietou_breakdown_text: lietou_breakdown_text.append("无付费渠道细分数据")
        else: lietou_breakdown_text.append("无付费渠道细分数据")
    else: lietou_breakdown_text.append("无猎头渠道入职")
    results['猎头'] = {"hires": lietou_hires, "percentage": lietou_percentage, "details_text_list": lietou_breakdown_text, "pie_data": {"labels": lietou_pie_labels, "values": lietou_pie_values}}

    # --- 4. 人才库盘活 (MODIFIED) ---
    talentpool_hires = hires_counts['人才库盘活']
    talentpool_percentage = (talentpool_hires / total_hires_for_percentage) * 100 if total_hires_for_percentage > 0 else 0
    ...
    if not talentpool_df.empty and talentpool_hires > 0:
        ...
        else: talentpool_breakdown_text.append("无付费渠道C类细分数据")
    else: talentpool_breakdown_text.append("无人才库盘活入职")
    results['人才库盘活'] = {"hires": talentpool_hires, "percentage": talentpool_percentage, "details_text_list": talentpool_breakdown_text, "pie_data": {"labels": talentpool_pie_labels, "values": talentpool_pie_values}}

    # --- 5. ST (交付团队 - 人才寻访组) ---
    st_hires = hires_counts['ST']
    st_percentage = (st_hires / total_hires_for_percentage) * 100 if total_hires_for_percentage > 0 else 0
    results['ST'] = {"hires": st_hires, "percentage": st_percentage, "details_text_list": ["交付团队-ST"], "pie_data": {}}

    # --- 6. 猎聘 (交付团队 - 高招猎聘) ---
    liepin_delivery_hires = hires_counts['猎聘']
    liepin_delivery_percentage = (liepin_delivery_hires / total_hires_for_percentage) * 100 if total_hires_for_percentage > 0 else 0
    results['猎聘'] = {"hires": liepin_delivery_hires, "percentage": liepin_delivery_percentage, "details_text_list": ["交付团队-猎聘"], "pie_data": {}}

    # --- Combined Pie for ST & 猎聘 ---
    st_lp_pie_labels, st_lp_pie_values = [], []
    if st_hires > 0: st_lp_pie_labels.append("ST (人才寻访组)"); st_lp_pie_values.append(st_hires)
    if liepin_delivery_hires > 0: st_lp_pie_labels.append("猎聘 (高招猎聘)"); st_lp_pie_values.append(liepin_delivery_hires)
